...
void ParticleSystem::render() const
{
    ...
    for (i = 0; i < num_particles; ++i)
    {
        ...
        glBegin(GL_QUADS); //TODO: use vertex array 1
...
void Snow::initFirst(Particle &particle)
{
    particle.pos = Point3f(randInRange(1.0), absRandInRange(1.0), randInRange(1.0));
    particle.velocity = Vector3f(0.0, -(absRandInRange(max_v - min_v) + min_v), 0.0);
}

void Snow::init(Particle &particle)
{
    particle.pos = Point3f(randInRange(1.0), 1.0, randInRange(1.0));
    particle.velocity = Vector3f(0.0, -(absRandInRange(max_v - min_v) + min_v), 0.0);
}
...
void Fireworks::render() const //TODO: alpha blend correctly :) 1
{
    if (_dead)
        return;
